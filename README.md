...
*   `DB_CLIENT=pg`: Клиент базы данных (например, `pg` для PostgreSQL).
...
*   `DB_NAME=task_tracker_db`: Имя базы данных.
*   `BACKEND_PORT=3000`: Порт, на котором будет работать приложение бэкенда NestJS.
...
*   `VITE_API_URL=http://localhost:3000`: Базовый URL для API бэкенда. При локальном запуске он обычно указывает на ваш сервис бэкенда. Если вы запускаете фронтенд напрямую (не через Nginx в Docker Compose), убедитесь, что это соответствует доступному URL вашего бэкенда.
...
Бэкенд NestJS будет доступен на порту `3000` (или на любом порту, который указан в `BACKEND_PORT` в вашем `.env`).
Пример: `http://localhost:3000/tasks`

## Доступ к фронтенду

Фронтенд Vue.js, обслуживаемый Nginx, будет доступен на порту `80` (или на любом порту, который вы сопоставили в `docker-compose.yml`).
Пример: `http://localhost:80`
